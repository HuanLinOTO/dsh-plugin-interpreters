...
var InterpretersCardController = class {
  ...
  async doSave() {
    ...
    try {
      ...
      if (!result.ok) {
        this.store.update((s) => {
          s.applyState = { kind: "error", message: result.error.message };
        });
        return;
      }
      const next = result.value.config;
      this.staged.clear();
      this.store.update((s) => {
        s.draft = { ...next };
        s.dirty = false;
...
var tagId = "@dsh-external/dsh-interpreters/InterpretersCard.module.css";
if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=" + JSON.stringify(tagId) + "]") === null) {
  const tag = document.createElement("style");
  tag.dataset.plugin = "@dsh-external/dsh-interpreters";
